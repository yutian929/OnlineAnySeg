...
"""读取若干个RGB-D帧，转成点云并提取每个点的feature(把所选各帧的点云concat到一起再提取特征，并进行颜色编码)"""

seq_name = "scene0011_00"
frame_id_list = [0, 1660]
...
def demo(config, input_pcd_list, output_dir=None):
    ...
    xyz_down, feature, indices_inverse = extract_features_mdfy(model, xyz=xyz_all, voxel_size=config.voxel_size, device=device, skip_check=True)
    feature_all = feature[indices_inverse]
