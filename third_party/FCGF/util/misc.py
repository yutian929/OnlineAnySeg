...
def extract_features_mdfy(model, xyz, rgb=None, normal=None, voxel_size=0.05, device=None, skip_check=False, is_eval=True):
  '''
  xyz is a N x 3 matrix
  rgb is a N x 3 matrix and all color must range from [0, 1] or None
  normal is a N x 3 matrix and all normal range from [-1, 1] or None
  ...
  # Voxelize xyz and feats
  v_coords = np.floor(xyz / voxel_size)
  # v_coords = xyz
  coords, inds, indices_inverse = ME.utils.sparse_quantize(v_coords, return_index=True, return_inverse=True)   #
  ...
  return return_coords, model(stensor).F, indices_inverse
